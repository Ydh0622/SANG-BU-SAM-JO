...
export const useConsultation = () => {
  ...
  // ✅ 업무 상태 변경 핸들러
  // 에러 해결: useEffect 대신 이벤트 핸들러 내부에서 상태를 직접 동기화합니다.
  const toggleWorkStatus = useCallback(() => {
    setStatus(prev => {
      const nextStatus = prev === 'OFFLINE' ? 'AVAILABLE' : 'OFFLINE';
      
      // 업무 종료 시 배정된 고객 정보도 함께 초기화 (Casdcading render 방지)
  ...
  // ✅ 상담 배정 시뮬레이션
  useEffect(() => {
    // AVAILABLE 상태가 아니거나 이미 고객이 배정된 경우 아무것도 하지 않음
    if (status !== 'AVAILABLE' || assignedCustomer) return;

    // 0.8초 ~ 2.5초 사이의 랜덤한 시간에 고객 배정
